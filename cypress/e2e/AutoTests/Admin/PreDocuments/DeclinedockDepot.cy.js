...
describe('pre document for decline dock to depot', () => {
    it('create new pre document in decline dock to depot', () => {
        cy.visit(`${URL}${admin}`)
        cy.wait(2000)
        ...
        let referencepage = new ReferencePage('/pre-documents', '/decline-dock-to-depot')
        ...
        cy.get('@get-dockId').its('response.body.id').then((res) => {
            ...
            cy.intercept('POST', `${URL}${admin_api}/inventory-document/decline-dock-to-depot/${res}/confirm`).as('confirm-dock')
